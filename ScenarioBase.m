classdef ScenarioBase < Common.MiniVieObj
    ...
    properties
        ...
        Verbose = 1;
        % Manoj
        OpenCOMPort = 0; %To ensure we only have to open once
        HandController; % Object for hand controller
    end
    ...
    methods
        ...
        function [className,prSpeed,rawEmg,windowData,features2D,voteDecision] = getIntentSignals(obj)
            ...
            if obj.Verbose > 0
                ...
                %%%%%%%%%%%Manoj
                if (obj.OpenCOMPort == 0)
                    fclose(instrfind);
                    obj.HandController = controller;
                    obj.HandController.openCOMPort();
                    obj.OpenCOMPort = 1;
                else
                    obj.HandController.controlHand(className);
                end
